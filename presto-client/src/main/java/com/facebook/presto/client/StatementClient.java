        ...
        implements Closeable
{
    ...
    private void processResponse(JsonResponse<QueryResults> response)
    {
        ...
        for (String addedPrepares : response.getHeaders().get(PRESTO_ADDED_PREPARE)) {
            List<String> keyValue = SESSION_HEADER_SPLITTER.splitToList(addedPrepares);
            if (keyValue.size() != 2) {
                continue;
            }
            addedPreparedStatements.put(keyValue.get(0), keyValue.get(1));
        }
        for (String deallocatedPrepares : response.getHeaders().get(PRESTO_DEALLOCATED_PREPARE)) {
            deallocatedPreparedStatements.add(deallocatedPrepares);
        }
