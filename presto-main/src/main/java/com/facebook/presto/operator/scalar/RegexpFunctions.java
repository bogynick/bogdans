...
public final class RegexpFunctions
{
    ...
    public static RegexpGenericPattern castToRegexp(ConnectorSession session, @SqlType(StandardTypes.VARCHAR) Slice pattern)
    {
        ...
        int dfaStatesLimit = session.getSystemProperty(RE2J_DFA_STATES_LIMIT, Integer.class);
        if (dfaStatesLimit < 2) {
        ...
        int dfaRetries = session.getSystemProperty(RE2J_DFA_RETRIES, Integer.class);
        if (dfaRetries < 0) {
