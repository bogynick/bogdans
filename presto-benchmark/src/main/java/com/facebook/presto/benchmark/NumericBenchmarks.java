...
public final class NumericBenchmarks
{
    ...
    {
        MaterializedResult result = localQueryRunner.execute("DESCRIBE lineitem");
        List<MaterializedRow> rows = result.getMaterializedRows();
        MaterializedRow extendedPriceRow = Iterables.find(rows, row -> row.getField(0).equals("extendedprice"));
        return localQueryRunner.getDefaultSession().getSchema() + "_" + extendedPriceRow.getField(1).toString();
    }
    ...
    {
        return createLocalQueryRunner(false, "sf0.1_decimal_" + precision + "_" + scale);
    }
