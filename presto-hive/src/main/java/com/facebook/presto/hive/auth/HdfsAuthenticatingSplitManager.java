        ...
        implements ConnectorSplitManager
{
    ...
    @Inject
    public HdfsAuthenticatingSplitManager(HadoopKerberosAuthentication authentication, HiveSplitManager targetSplitManager)
    {
    ...
    {
        return authentication.doAs(session.getUser(), () -> targetSplitManager.getPartitions(session, table, tupleDomain));
    }
    ...
    {
        return authentication.doAs(session.getUser(), () -> targetSplitManager.getPartitionSplits(session, table, partitions));
    }

    @Override
    public ConnectorSplitSource getSplits(ConnectorSession session, ConnectorTableLayoutHandle layout)
    {
        return authentication.doAs(session.getUser(), () -> targetSplitManager.getSplits(session, layout));
    }
}
